function [Ps,Pt]=updateG_ker(Ps0,Pt0,Xs,Xt,K,Ys,Yt_f,beta1,beta2,options) 
%用梯度下降的方式求
d       = options.d;
alpha   = options.alpha;
...
for iter = 1:Tg      
    ...
    if (iter == 1)
        ...
        A  = 2*alpha*K+2*lambdaMMD*Mt;
        B  = 2*lambda*Xt*Xt';
        C  = beta1*beta1'-beta2*beta1'-beta1*beta2'+beta2*beta2';
        D  = 2*alpha*K*Ps-lambdaMMD*Mts*Ps-lambdaMMD*Mst'*Ps;
...
%         Ps = Ps0;
%     else
%          Ps = (2*alpha*eye(m)+lambdaMMD*Ms)\(2*alpha*Pt-lambdaMMD*Mts'*Pt-lambdaMMD*Mst*Pt);+eye(SIZE11,d)
         SSSS=inv(2*alpha*K+2*lambdaMMD*Ms+Ipara*eye(m));
         SIZE11=size(Ms,1);
         SSSS1=(2*alpha*K*Pt-lambdaMMD*Mts'*Pt-lambdaMMD*Mst*Pt);
         Ps = SSSS*SSSS1;
%     end       
